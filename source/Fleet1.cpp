...
Fleet1::Fleet1(reservoir_fleet R, double gamma) {
	this->gamma = gamma;
...
double Fleet1::ran_double() { return dbl_ran(gen); }

void Fleet1::process_edge(int A, int B) {
	this->time_step++;
	if ((this->R.get_current_size() == this->R.get_maximum_size())) {
		...
		long double inv_p = 1. / this->prob;
		long long bfly = (long long)BFC_in_reservoir(R);
...
void Fleet1::process_stream(int itr, stream& S) {
	this->prob = 1.;
	this->time_secs = 0.;
	this->time_step = 0;
