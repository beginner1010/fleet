...
Fleet3::Fleet3(reservoir_fleet R, double gamma) {
	this->gamma = gamma;
...
double Fleet3::ran_double() { return dbl_ran(gen); }

void Fleet3::process_edge(int A, int B) {
	this->time_step++;
	double inv_p = 1. / this->prob;
	long long bfly = bfc_per_edge_ada(A, B, R);
	this->seen_bfly += bfly;
	this->global_counter += bfly * inv_p * inv_p * inv_p;
...
void Fleet3::process_stream(int itr, stream& S) {
	this->prob = 1.;
	this->time_secs = 0.;
	this->time_step = 0;
